...
if (!$demomode) {
  if (!is_writable($dbfile)) {
		...
		echo "<b><big>Make $dbfile writeable by the user running the web server</b></big><br>";
  ...
  if (!is_writable("$scriptdir/data")) {
	...
	echo "<b><big>Make $scriptdir/data writeable by the user running the web server</b></big><br>";
  ...
  */
  ...
  if (!is_writable($uploaddir)) {
	echo "$scriptdir is not writeable by apache<br>";
	echo "<b><big>Make $uploaddir writeable by the user running the web server</b></big><br>";
  ...
  if (!is_writable("$scriptdir/translations")) {
	...
	echo "<b><big>Make $scriptdir/translations writeable by the user running the web server</b></big><br>";
...
$timeparam = $settings['timeformat']; 
if (!empty($timeparam)) {
    $timeparam = " $timeparam"; // 结果会是 " H:i:s"
}
...
if (!$demomode ) {
  ...
  elseif (isset($_COOKIE["itdbuser"])) { //& isset itdbookie1, check if valid
    ...
    if ($userdata[0]['cookie1']==$_COOKIE["itdbcookie1"]) {
      $authstatus=1;
      $authmsg=t("Welcome back ").$_COOKIE["itdbuser"];
      setcookie("itdbcookie1",$userdata[0]['cookie1'], time()+3600*24*2,$wscriptdir); //renew for two days
    }
    else {
      $authstatus=0;
      $authmsg=t("Logged on from another browser? Please re-login.");
    }
  }
  else
    $authmsg=t("Please Login!");
}
elseif ($demomode) { //demomode
  $authstatus=1;
  $authmsg=t("Demo mode, no save possible");
}
