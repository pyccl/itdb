...
if ($nextstep==1 && strlen($_FILES['file']['name'])>2) { //insert file
  ...
  $result = '';
	...
	if ($file_ext !== 'csv') {
	    die(t("Error: Only CSV files are allowed."));
	}
...
<?php if ($nextstep == 0) { ?>
    <h1><?php te("Experimental import"); ?></h1>
    <h2><?php te("BACKUP your ITDB FIRST!"); ?></h2>
		...
		<script type="text/javascript">
		    function validateForm() {
		        ...
		        // 1. 检查是否选择了文件
		        if (fileInput.value == '') {
		            alert('<?php te("Please select a file to upload"); ?>');
		            return false;
		        }
		        ...
		        if (extension !== 'csv') {
		            alert('<?php te("Invalid file format. Only CSV files are allowed."); ?>');
		            return false; // 阻止提交
		        }
    ...
    <!-- 修改点：添加 onsubmit="return validateForm()" -->
    <form method=post name='importfrm' action='<?php echo $scriptname?>?action=<?php echo $action?>' enctype='multipart/form-data' onsubmit="return validateForm()">
        <tr>
            <td><?php te("File"); ?>:</td>
            <td>
        ...
        <tr>
            <td style='font-weight:bold'><?php te("Delimiter"); ?>:</td>
            <td>
                <input size=1 type=text name='delim' value=';' maxlength=1><?php te("(field separator)"); ?>
            </td>
        </tr>
        <tr>
            <td><?php te("Skip 1st row"); ?>:</td>
            <td>
                <select name=skip1st>
                    <option value=1><?php te("Yes"); ?></option>
        ...
        <tr>
            <td colspan=2>
                <input type=submit value='<?php te("Upload file and inspect fields"); ?>'>
            </td>
        </tr>
...
<p>
<?php te("Expected format is CSV file with the following fields"); ?>:<br>
<big>
<p>
    <?php
    $sep="";
...
?>
	...
	<br><h2><?php te("Please check fields for consistency before submiting"); ?></h2>
	...
	foreach ($imlines as $line_num => $line) {
		...
		$cols=explode($delim,$line);
			if (count($cols) != $nfields) { 
			    echo sprintf(
			        t("<b><big>Error: field count in line %d is %d, %d is expected</big></b>"), 
			        $line_num, 
			        count($cols), 
			        $nfields
			    ); 
	...
	<div style='float:left;clear:both; width:100%; margin-top:20px;'>
		...
		<b><?php te("New H/W Manufacturers detected (will be inserted to the DB)"); ?>:</b><br>
		<hr>
		<?php 
		...
		<b><?php te("New Users detected (will be inserted to the DB)"); ?>:</b><br>
		<hr>
		<?php
	    ...
	    <div style='border:1px solid #ccc;width:200px;height:200px;overflow:auto; text-align:left;float:left;margin-left:20px;'>
		<b><?php te("New Item Types detected (will be inserted to the DB)"); ?>:</b><br>
		<hr>
		<?php
	    ...
	    <div style='border:1px solid #ccc;width:200px;height:200px;overflow:auto; text-align:left;float:left;margin-left:20px;'>
		<b><?php te("Invalid Status Types detected (will NOT be inserted to the DB)"); ?>:</b><br>
		<hr>
		<?php
	    ...
	    <div style='border:1px solid #ccc;width:200px;height:200px;overflow:auto; text-align:left;float:left;margin-left:20px;'>
		<b><?php te("New Locations / Locarea detected (will be inserted to the DB)"); ?>:</b><br>
		<hr>
		<?php
	...
	<div style='clear:both;text-align:center:width:100%; '>
		...
		<td colspan=2><input type=submit value='<?php te("Import"); ?>' ></td></tr>
		...
		<td colspan=2><input type=submit value='<?php te("Go back"); ?>' ></td></tr>
		</form>
	</div>
...
if ($nextstep==2) {
	...
	echo t(sprintf('<b>Updating DB with %s</b>', $imfn));
	...
	foreach ($imlines as $line_num => $item) {
		if ($line_num==0 && $_POST['skip1st']) {
			echo t("<br>Skipping first line<br>");
			continue;
		}
		...
		$lr=getlocidsbynames($cols[$name2fno['location']],$cols[$name2fno['area']]);
		if ($lr[0]<0) {
			echo t(sprintf('Location/locarea non existent: %s/%s<br>', $cols[$name2fno['location']], $cols[$name2fno['area']]));
			$locid="";
			$locareaid="";
		}
		else {
...
function lineok ($line,$delim) {
	...
	if (!strlen($cols[$name2fno['ip']])  //ip
        ...
        echo "\n";
		// 1. 跳过半空行
		echo t(sprintf('Skipping semi-empty line (%s)<br>', $line));
		
		// 2. 制造商信息
		echo t(sprintf('Manuf: %s <br>', $cols[$name2fno['manufacturer']]));
		
		// 3. 型号信息
		echo t(sprintf('Model: %s <br>', $cols[$name2fno['model']]));
		
		// 4. 分隔符信息
		echo t(sprintf('Delim: %s<br>', $delim));
		
		// 5. 数组内容打印 (print_r 返回字符串)
		echo t(sprintf('cols: %s<br>', print_r($cols, true)));
