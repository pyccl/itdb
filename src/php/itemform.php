...
$(document).ready(function() {
	// ===================== 机架式显示隐藏（你原来的，完全不动） =====================
	function refreshRackFields() {
		let isRack = $("input[name='rackmountable']:checked").val() === "1";
		if (isRack) {
			$("#locationid").closest("tr").show();
			$("#locareaid").closest("tr").show();
			$("#rackid").closest("tr").show();
			$("#rackposition").closest("tr").show();
			$("select[name='usize']").closest("tr").show();
		} else {
			$("#locationid").closest("tr").hide();
			$("#locareaid").closest("tr").hide();
			$("#rackid").closest("tr").hide();
			$("#rackposition").closest("tr").hide();
			$("select[name='usize']").closest("tr").hide();
		}
	}
	...
	// 绑定事件
	$("input[name='rackmountable']").change(refreshRackFields);
	$("select[name='itemtypeid']").change(refreshSoftwareTab);
...
// 检查 URL 是否带有 sourceid 参数，且当前是新建模式
if ($id == "new" && isset($_GET['sourceid']) && is_numeric($_GET['sourceid'])) {
    $sourceid = intval($_GET['sourceid']);
    $clone_mode = true;

    // 读取源数据
    $sql = "SELECT * FROM items WHERE id = $sourceid";
    $sth = db_execute($dbh, $sql);
    $source_item = $sth->fetchAll(PDO::FETCH_ASSOC);

    if (count($source_item) > 0) {
        // 将源数据的值赋给表单变量 (模拟 $_POST 的效果)
        foreach ($formvars as $formvar) {
            if (isset($source_item[0][$formvar])) {
                // 特殊处理：克隆时通常不需要保留原 ID 的关联状态
                // 如果有特殊字段不想克隆（比如 internalid 强制留空），可以在这里覆盖
                // 例如：如果想让 Internal ID 在克隆时自动清空，取消下一行的注释
                // if ($formvar == 'internalid') continue; 
                
                $$formvar = $source_item[0][$formvar];
            }
        }
        // 克隆模式下，Internal ID 通常需要清空或标记
        // 这里选择清空，强制用户输入新的 Internal ID
        $internalid = "";
    }
}
// === 结束新增 ===
...
if ($id == "new") { 
    ...
    // 如果是克隆模式，我们保留 edititem.php 中已经赋好的值（来自 sourceid 的数据）
...
<h1> 
    ...
    echo $caption; 
    // 显示克隆模式提示
    if (isset($clone_mode) && $clone_mode) { 
        echo " <span style='color:red;font-size:0.8em'>[".t("Clone Mode: Data not saved yet")."]</span>"; 
    } 
    ?> 
</h1>
...
<div id="tab1" class="tab_content">
  ...
  <td class='tdtop'>
	...
	<!-- === 新增：内部编号字段 === -->
	<tr>
	    <td class='tdt'><?php te("Internal ID");?>:</td>
	    <td>
			<input class='tdt' type='text' title='<?php te("(Optional) Internal Reference Number");?>' value='<?php echo htmlspecialchars($internalid, ENT_QUOTES, "UTF-8")?>' name='internalid' id='internalid' maxlength='50'>
	    </td>
	</tr>
	<!-- === 结束新增 === -->
    ...
    ?>
      ...
      <td title='<?php te("Select yes for parts/components");?>'>
      <div class='mandatory'>
    ...
    ?>
      ...
      <td>
      <div class=mandatory>
		<input id="rack_yes" validate='required:true' class='radio' <?php echo $y?> type=radio name='rackmountable' value='1'><?php te("Yes");?>
		<input id="rack_no" class='radio' type=radio <?php echo $n?> name='rackmountable' value='0'><?php te("No");?>
    ...
    <?php   if (is_numeric($manufacturerid)) 
      echo "<a title='".t("Edit selected manufacturer (agent)")."' href='$scriptname?action=editagent&amp;id=$manufacturerid'><img src='images/edit.png'></a> "; ?>
    ...
    if (isset($manufacturerid)) {
      ...
      if (strlen($st)) $st="<a target=_blank href='$st'>".t("Service Tag")."</a>:";
      else $st=t("Service Tag").":";
    }
    ?>
	...
	echo "<option $s value='$dbid'>$itype</option>\n";
      ...
      ?>
		...
		<?php
		// 读取部门（和编辑部门页面完全一样）
		$sql_all = "SELECT id, name, parent_id FROM departments ORDER BY sort_order, name";
		$sth_all = db_execute($dbh, $sql_all);
		$all_depts = $sth_all->fetchAll(PDO::FETCH_ASSOC);
		
		$deptTree = buildTree($all_depts);
		
		// 读取员工
		$emp_stmt = $dbh->query("SELECT id, name, department_id FROM employees ORDER BY name ASC");
		$employees = $emp_stmt->fetchAll(PDO::FETCH_ASSOC);
		?>
		<tr>
		    <td class='tdt'><?php te("Department");?>:</td>
		    <td>
		        <select name="custom_dept" id="custom_dept" style="width:155px;">
		            <option value=""><?php te("Select"); ?></option>
		            <?php
		            foreach ($deptTree as $dept) {
		                $depth = $dept['depth'];
		                // ########## 和 editdepartments 完全一样的缩进规则 ##########
		                if ($depth == 0) {
		                    $prefix = '';
		                } else {
		                    $prefix = str_repeat('　　', $depth - 1) . '┕┉';
		                }
		                $selected = ($dept['id'] == $custom_dept) ? 'selected' : '';
		...
		<tr>
		    <td class='tdt'><?php te("End User");?>:</td>
		    <td>
		        <select name="custom_user" id="custom_user" style="width:155px;">
		            <option value=""><?php te("Select"); ?></option>
		        </select>
		    </td>
		</tr>
		...
		{ id: "<?php echo $e['id']; ?>", name: "<?php echo addslashes($e['name']); ?>", deptId: "<?php echo $e['department_id']; ?>" },
		<?php endforeach; ?>
		];
		
		var current_uid = "<?php echo addslashes($custom_user); ?>";
		
		setTimeout(function(){
		    var deptSelect = document.getElementById('custom_dept');
		    var userSelect = document.getElementById('custom_user');
		
		    function reloadUsers() {
		        var deptId = deptSelect.value;
		        userSelect.innerHTML = '<option value=""><?php te("Select"); ?></option>';
		        if (!deptId) return;
		        ...
		        for (var i = 0; i < allEmployees.length; i++) {
		            ...
		            if (emp.deptId == deptId) {
		                ...
		                if (emp.id == current_uid) opt.selected = true;
		                userSelect.appendChild(opt);
		            }
		        }
		    }
		
		    deptSelect.onchange = reloadUsers;
		    reloadUsers();
		}, 100);
		</script>
		<!-- 自定义使用人与部门 End -->
	...
	<select id='locareaid' name='locareaid'>
      ...
      <tr>
	<?php
	// --- Rack Selection with Conditional Buttons ---
	// 初始化按钮变量
	$rack_buttons = "";
	
	// 只有当 $id 不是 "new" (即编辑模式) 且 $rackid 是数字时，才生成按钮
	if ($id != "new" && is_numeric($rackid) && $rackid > 0) {
	    $rack_buttons = "<a id='viewrack_link' alt='".t("View")."' title='".t("view rack")."' href='$scriptname?action=viewrack&amp;id=$rackid&amp;highlightid=$id&amp;nomenu=1'>
	                        <img height=12 src='images/eye.png'>
	                     </a> "
	                     . "<a alt='".t("Edit")."' title='".t("Edit Rack")."' href='$scriptname?action=editrack&amp;id=$rackid&amp;highlightid=$id'>
	                         <img src='images/edit.png' style='vertical-align: middle; margin-left: 5px;'>
	                     </a>";
	}
	?>
	<!-- Display Label and Buttons -->
	<td class='tdt'>
	    <?php echo $rack_buttons; // 输出按钮（如果条件满足） ?>
	    <?php te("Rack");?>:
	    <script type="text/javascript">
	        // 仅在按钮存在时初始化弹窗
	        <?php if ($rack_buttons != "") { ?>
	        $('a#viewrack_link').popupWindow({
	            centerScreen:1, 
	            height:800, 
	            scrollbars:1, 
	            width:700, 
    ...
    <select id='rackid' name='rackid'>
      ...
      <td class='tdt' class='tdt'><?php te("Rack Pos. (topmost)");?>:</td><td>
		...
		<select id="rackposition" name='rackposition' title='<?php te("Rack Row");?>' style='width:40%'>
		<option value=''><?php te("Select");?></option>
		</select>
	
	<script>
	// 机柜位置动态生成：根据选中机柜的高度自动生成 1~N
	function refreshRackPosByRackU() {
	    var rackId = $('#rackid').val();
	    var rackPosSel = $('#rackposition');
	    rackPosSel.empty().append('<option value=""><?php te("Select");?></option>');
	
	    var maxU = 0;
	
	    // 从选中机柜的文本里提取 U 数
	    var rackText = $('#rackid option:selected').text();
	    var match = rackText.match(/(\d+)U/i);
	    if (match && match[1]) {
	        maxU = parseInt(match[1]);
	    }
	
	    // 生成 1 ~ maxU
	    for (var i = 1; i <= maxU; i++) {
	        var selected = "<?php echo $rackposition; ?>" == i ? "selected" : "";
	        rackPosSel.append('<option ' + selected + ' value="' + i + '">' + i + '</option>');
	    }
	}
	
	// 页面加载 & 切换机柜时刷新
	$(document).ready(function() {
	    refreshRackPosByRackU();
  ...
  ?>
	...
	<span id="softwareTabTitle" class="tita" onclick='showid("software");'><?php te("Software");?></span> |
...
$error = $dbh->errorInfo();if($error[0] && isset($error[2])) echo t("Error")." ei0:".$error[2]."<br>";
...
while ($r=$sth->fetch(PDO::FETCH_ASSOC)) {
  ...
  echo "\n <tr><td><input name='itlnk[]' value='".$r['id'].
   ...
   "<td class='bld'><a title='".sprintf(t("Edit item %s in new window"),$r['id'])."' ".
...
while ($r=$sth->fetch(PDO::FETCH_ASSOC)) {
  ...
  echo "\n  <tr><td><input name='itlnk[]' value='".$r['id'].
   "' type='checkbox' /></td>".
   "<td ><a title='".sprintf(t("Edit item %s in new window"),$r['id'])."' ".
...
<div id='tab3' class='tab_content'><!-- Invoices -->
  ...
  <table border='0' class=tbl2 style='border-bottom:1px solid #cecece;width:100%;'>
    <tr>
    <td colspan=2 style='text-align:center;' title='<?php te("Select invoice(s) related to this item");?>'>
      <h2>
...
if (isset($id) && strlen($id)) { //show actions
  ...
  echo "<br><p><b>".t("Item Log can be edited after inserting item")."</b>";
}
?>
</div><!-- tab4 -->
...
$xx=0;
  while ($ir=$sth->fetch(PDO::FETCH_ASSOC)) {
    ...
    echo  " type='checkbox' /></td>".
     "<td $cls><a class='editid' title='".sprintf(t("Edit software %s in new window"),$ir['id'])."' ".
...
<div id='tab6' class='tab_content'>
  ...
  while ($ir=$sth->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($ir['islinked']) echo " checked ";
    echo  " type='checkbox' /></td>".
     "<td $cls><a class='editid' title='".sprintf(t("Edit contract %s in new window"),$ir['id'])."' ".
...
<table><!-- save buttons -->
<tr>
<td style='text-align: center' colspan=1><button type="submit"><img src="images/save.png" alt='<?php te("Save");?>' > <?php te("Save");?></button></td>
<?php 
if ($id!="new") {
...
// 修改克隆按钮：使用 sourceid 模式，而不是 cloneid 模式
  echo "\n<td style='text-align: center' ><button type='button' onclick='javascript:cloneconfirm(\"Item {$_GET['id']}\",\"$scriptname?action=$action&amp;id=new&amp;sourceid={$_GET['id']}\");'>". "<img src='images/copy.png' border=0>". t("Clone")."</button></td>\n";
} 
else 
  echo "\n<td>&nbsp;</td>";
?>
