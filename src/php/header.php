...
<html>
<head>
<title>ITDB - [<?php  echo $title." $id";?>] - <?php te("IT Items Database");?></title>
...
function delconfirm(what,delurl) {
  ...
  var answer = confirm("<?php echo addslashes(t("Are you sure you want to delete id ")); ?>" + what + " ?")
  if (answer) window.location = delurl;
}

function cloneconfirm(what,cloneurl) {

  var answer = confirm("<?php echo addslashes(t("Are you sure you want to clone item id ")); ?>" + what + " ?")
...
function delconfirm2(what,delurl,msg)
{
  ...
  if (arguments.length<3) 
    mesg="<?php echo addslashes(t("Warning! All associations and orphaned files will be removed too. Write YES if you want to delete this : ")); ?>" + what;
  else 
    mesg=msg+"<?php echo addslashes(t("Write YES if you want to delete id ")); ?>"+what;

  $i=window.prompt(mesg,'NO');
  if ($i=='YES') 
    window.location = delurl;
  else
    alert("<?php echo addslashes(t("ABORTED...")); ?>");
}
