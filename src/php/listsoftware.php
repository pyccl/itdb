...
while ($r=$sth->fetch(PDO::FETCH_ASSOC)) {
  ...
  for ($invinfo="",$i=0;$i<$ninv;$i++) {
    ...
    for ($flnk="",$c=0;$c<count($f);$c++) {
       $fname=$f[$c]['fname'];
       $ftitle=$f[$c]['title'];
       $flnk.=" <a target=_blank title='".t("View FILE").": $ftitle $fname' href='".$uploaddirwww.$fname."'><img src='images/down.png'></a>";
    }
    $invinfo.="<div style='min-width:70px;'><a title='".t("Edit Invoice")."' href='$scriptname?action=editinvoice&amp;id=$iid'><div class='editid'>$iid</div><div>$flnk</div></a></div>";
    $flnk="";
  }
