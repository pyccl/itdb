...
global $dbh,$uploadErrors;
    ...
    if(!$_FILES[$file_id]['size']) return array('',t("File is zero length"));
    if(!is_numeric($ftype)) return array('',t("No type specified")." ($ftype)");
    if(!strlen($title)) return array('',t("No title specified"));
    if(!strlen($date)) return array('',t("No date specified"));
    if(!strlen($assoctable)) return array('',t("No table specified"));
    if(!strlen($colname)) return array('',t("No colname specified"));
    if(!strlen($folder)) return array('',t("No folder specified"));
    ...
    if (!move_uploaded_file($_FILES[$file_id]['tmp_name'], $uploadfile)) {
        $result = t("Cannot upload the file")." '".$_FILES[$file_id]['name']."'"; 
