<?php
	 ...
	 */
	$aColumns = array('itemid','internalid','itemlabel','typedesc','title','itemmodel','dnsname','serial','purchasedate',
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			if ( $aColumns[$i] == "itemid" ) {
				...
				$r="<div style='width:60px'><span $attr>&nbsp;</span>".
				   "<span><a class='editid' title='".t("Edit")."' href='?action=edititem&amp;id=".$aRow['itemid']."'>".
			...
			elseif ( $aColumns[$i] == "remdays" ) {
				...
				$rd=$remdays_r['days'];
				$row[] = "<small><div title='$rd".t("Days")."'>". $rdstr. "</div></small>"; // title attribute used for sorting
			}
